...
export const AgentExecution: React.FC<AgentExecutionProps> = ({
  ...
  const rowVirtualizer = useVirtualizer({ count: displayableMessages.length, getScrollElement: () => scrollContainerRef.current, estimateSize: () => 150, overscan: 5 });
  const fullscreenRowVirtualizer = useVirtualizer({ count: displayableMessages.length, getScrollElement: () => fullscreenScrollRef.current, estimateSize: () => 150, overscan: 5 });
  ...
  const isAtBottom = () => {
    ...
    if (container) { const { scrollTop, scrollHeight, clientHeight } = container; return scrollHeight - scrollTop - clientHeight < 80; }
    return true;
  };

  useEffect(() => {
    if (displayableMessages.length === 0 || !scrollLocked) return;
    if (isFullscreenModalOpen) { fullscreenRowVirtualizer.scrollToIndex(displayableMessages.length - 1, { align: "end", behavior: "auto" }); }
    else { rowVirtualizer.scrollToIndex(displayableMessages.length - 1, { align: "end", behavior: "auto" }); }
    requestAnimationFrame(() => {
      const container = isFullscreenModalOpen ? fullscreenScrollRef.current : scrollContainerRef.current;
      if (container && scrollLocked) { container.scrollTop = container.scrollHeight; }
    });
  ...
  useEffect(() => {
    if (isRunning && executionStartTime) {
      elapsedTimeIntervalRef.current = setInterval(() => { setElapsedTime(Math.floor((Date.now() - executionStartTime) / 1000)); }, 1000);
    } else { if (elapsedTimeIntervalRef.current) clearInterval(elapsedTimeIntervalRef.current); }
  ...
  const handleClaudeError = useCallback((e: Event) => {
    ...
    trackEvent.agentError({ error_type: 'runtime_error', error_stage: 'execution', retry_count: 0, agent_type: agent.name || 'custom' });
  ...
  useEffect(() => { return () => { removeListeners(); }; }, [removeListeners]);
  ...
  const handleExecute = async () => {
    try {
      ...
      setMessages([]); setRawJsonlOutput([]); setError(null);
      addListeners();
      const connId = await initAgentSession({ agentName: agent.name, projectPath, prompt: task, model, permissionMode });
      connectionIdRef.current = connId;
      trackEvent.agentStarted({ agent_type: agent.name || 'custom', agent_name: agent.name, has_custom_prompt: !!task.trim() });
      agentFeatureTracking.trackUsage();
    } catch (err) {
      removeListeners();
      console.error("Failed to execute agent:", err);
      setIsRunning(false); setExecutionStartTime(null); executionStartTimeRef.current = null;
      if (tabId) updateTabStatus(tabId, 'error');
      setMessages(prev => [...prev, { type: "result", subtype: "error", is_error: true, result: `Failed to execute agent: ${err instanceof Error ? err.message : 'Unknown error'}`, duration_ms: 0, usage: { input_tokens: 0, output_tokens: 0 } }]);
    }
  };

  const handleStop = async () => {
    const connId = connectionIdRef.current;
    if (connId) { interruptSession(connId); }
    setIsRunning(false); setExecutionStartTime(null); executionStartTimeRef.current = null;
  ...
  const handleBackWithConfirmation = () => {
    if (isRunning) {
      const shouldLeave = window.confirm("An agent is currently running. If you navigate away, the agent will continue running in the background.\n\nDo you want to continue?");
      if (!shouldLeave) return;
    }
    if (connectionIdRef.current && !isRunning) { closeSessionSocket(connectionIdRef.current); }
    onBack();
  };
  ...
  const scrollHandler = () => {
    ...
    scrollRafRef.current = requestAnimationFrame(() => {
      ...
      if (atBottom && !scrollLocked) setScrollLocked(true);
      else if (!atBottom && scrollLocked) setScrollLocked(false);
    });
  };
    ...
    <div className="relative w-full" style={{ height: `${virtualizer.getTotalSize()}px` }}>
      <AnimatePresence>
        {virtualizer.getVirtualItems().map((virtualItem) => {
          ...
          return (
            <motion.div key={virtualItem.key} data-index={virtualItem.index} ref={(el) => { if (el) virtualizer.measureElement(el); }} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="absolute inset-x-4 pb-4" style={{ top: virtualItem.start }}>
              <ErrorBoundary><StreamMessage message={message} streamMessages={messages} /></ErrorBoundary>
            </motion.div>
          );
        })}
      </AnimatePresence>
  ...
  const renderScrollControl = (targetVirtualizer: Virtualizer<HTMLDivElement, Element>, targetScrollRef: React.RefObject<HTMLDivElement | null>, keyPrefix: string) => (
    <AnimatePresence mode="wait">
      {displayableMessages.length > 0 && scrollLocked && (
        <motion.div key={`${keyPrefix}-locked`} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }} transition={{ duration: 0.12 }} className="absolute bottom-3 right-3 z-20">
          <button onClick={() => setScrollLocked(false)} title="Auto-scroll on — click to unlock" className="p-1.5 rounded-full text-primary/40 hover:text-primary/70 hover:bg-primary/10 transition-colors"><Lock className="h-3 w-3" /></button>
        </motion.div>
      )}
      {displayableMessages.length > 0 && !scrollLocked && (
        <motion.div key={`${keyPrefix}-unlocked`} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 8 }} transition={{ duration: 0.15 }} className="absolute bottom-3 right-3 z-20">
          <button onClick={() => { setScrollLocked(true); targetVirtualizer.scrollToIndex(displayableMessages.length - 1, { align: 'end', behavior: 'auto' }); requestAnimationFrame(() => { const el = targetScrollRef.current; if (el) el.scrollTop = el.scrollHeight; }); }} title="Scroll to bottom" className="flex items-center gap-1 px-2.5 py-1.5 rounded-full text-[10px] font-medium border shadow-lg backdrop-blur-sm bg-primary/15 text-primary border-primary/25 hover:bg-primary/25 transition-all">
    ...
    <div className={cn("flex flex-col h-full bg-background", className)}>
      <div className="h-full flex flex-col bg-background">
        ...
        <div className="p-6 border-b border-border">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Button variant="ghost" size="icon" onClick={handleBackWithConfirmation} className="h-9 w-9 -ml-2" title="Back"><ArrowLeft className="h-4 w-4" /></Button>
              <div>
            ...
            <div className="flex items-center gap-2">
              {messages.length > 0 && (
                <Button variant="outline" size="default" onClick={() => setIsFullscreenModalOpen(true)}><Maximize2 className="h-4 w-4 mr-2" />Fullscreen</Button>
              )}
            </div>
          </div>
        </div>

        {/* Configuration Section — extracted */}
        <AgentControls
          task={task} onTaskChange={setTask}
          model={model} onModelChange={setModel}
          isRunning={isRunning} projectPath={projectPath} error={error}
          isolation={isolation} onIsolationChange={setIsolation}
          runInBackground={runInBackground} onRunInBackgroundChange={setRunInBackground}
          permissionMode={permissionMode} onPermissionModeChange={setPermissionMode}
          onExecute={handleExecute} onStop={handleStop}
        />
        ...
        <div className="flex-1 overflow-hidden relative">
              ...
              <div ref={messagesContainerRef}>
                ...
                {isRunning && messages.length === 0 && (
                  <div className="flex items-center justify-center h-full">
                    <div className="flex items-center gap-3"><Loader2 className="h-6 w-6 animate-spin" /><span className="text-sm text-muted-foreground">Initializing agent...</span></div>
                  </div>
                )}
          ...
          {renderScrollControl(rowVirtualizer, scrollContainerRef, "main")}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-[60] bg-background flex flex-col">
          <div className="flex items-center justify-between p-4 border-b border-border">
            <div className="flex items-center gap-2">
              ...
              {isRunning && (<div className="flex items-center gap-1"><div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div><span className="text-xs text-green-600 font-medium">Running</span></div>)}
            </div>
            <div className="flex items-center gap-2">
              <Popover trigger={<Button variant="ghost" size="sm" className="flex items-center gap-2"><Copy className="h-4 w-4" />Copy Output<ChevronDown className="h-3 w-3" /></Button>} content={<div className="w-44 p-1"><Button variant="ghost" size="sm" className="w-full justify-start" onClick={handleCopyAsJsonl}>Copy as JSONL</Button><Button variant="ghost" size="sm" className="w-full justify-start" onClick={handleCopyAsMarkdown}>Copy as Markdown</Button></div>} open={copyPopoverOpen} onOpenChange={setCopyPopoverOpen} align="end" />
              <Button variant="ghost" size="sm" onClick={() => setIsFullscreenModalOpen(false)} className="flex items-center gap-2"><X className="h-4 w-4" />Close</Button>
            </div>
          </div>
          <div className="flex-1 overflow-hidden p-6 relative">
            <div ref={fullscreenScrollRef} className="h-full overflow-y-auto space-y-8" onScroll={scrollHandler}>
              {messages.length === 0 && !isRunning && (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  <Terminal className="h-16 w-16 text-muted-foreground mb-4" /><h3 className="text-lg font-medium mb-2">Ready to Execute</h3><p className="text-sm text-muted-foreground">Enter a task to run the agent</p>
                </div>
              )}
              {isRunning && messages.length === 0 && (
                <div className="flex items-center justify-center h-full"><div className="flex items-center gap-3"><Loader2 className="h-6 w-6 animate-spin" /><span className="text-sm text-muted-foreground">Initializing agent...</span></div></div>
              )}
              {renderVirtualList(fullscreenRowVirtualizer)}
              <div ref={fullscreenMessagesEndRef} />
            </div>
            {renderScrollControl(fullscreenRowVirtualizer, fullscreenScrollRef, "fs")}
          </div>
        </div>
      )}
    </div>
